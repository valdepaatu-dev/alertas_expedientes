...
es_admin = False
if modo_admin:
    # Puedes cambiar "atu2026" por la contraseña que tú prefieras
    contrasena = st.sidebar.text_input("Contraseña de Administrador:", type="password")
...
if es_admin:
    ...
    archivo_subido = st.sidebar.file_uploader("Deposite el nuevo archivo Excel aquí", type=["xlsx"])
    ...
    st.sidebar.info("💡 Los usuarios generales solo pueden visualizar los reportes. El botón de carga está oculto.")

# --- PROCESAMIENTO Y VISUALIZACIÓN DEL REPORTE ---
# Revisa si existe el archivo maestro (ya sea el subido en la sesión o el que está guardado en GitHub)
if os.path.exists(ARCHIVOMAESTRO):
    ...
    st.info("👋 Bienvenido. Esperando que el administrador realice la primera carga del archivo base.")